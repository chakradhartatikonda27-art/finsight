...
export default function UploadPage() {
  ...
  return (
    <Layout>
      <div style={{ background:'#fff', borderBottom:'1px solid #E5E7EB', padding:'0 24px', height:52, display:'flex', alignItems:'center', fontFamily:'system-ui', position:'sticky', top:0, zIndex:40 }}>
        <span style={{ fontWeight:700, color:'#1F3864', fontSize:14 }}>Upload Tally Data</span>
        <span style={{ color:'#6B7280', fontSize:12, marginLeft:12 }}>Upload XLS → Parse → Save to Supabase</span>
      </div>
      ...
      <div style={{ padding:'32px 24px', fontFamily:'system-ui', maxWidth:680 }}>
        ...
        >
          ...
          {file ? (
            ...
            <>
              <div style={{ fontSize:40, marginBottom:12 }}>📂</div>
              <div style={{ fontSize:16, fontWeight:600, color:'#374151', marginBottom:8 }}>Drop your Tally Day Book here</div>
              <div style={{ fontSize:12, color:'#6B7280', marginBottom:16 }}>XLS · XLSX · CSV · Max 50 MB</div>
              <div style={{ display:'inline-block', background:'#1F3864', color:'#fff', padding:'8px 20px', borderRadius:8, fontSize:13, fontWeight:600 }}>Browse File</div>
            </>
          )}
        </div>
        ...
        {/* Upload button */}
        {file && step === 'idle' && (
          <button onClick={runUpload} style={{ width:'100%', background:'#1F3864', color:'#fff', border:'none', borderRadius:10, padding:'14px', fontSize:15, fontWeight:700, cursor:'pointer', marginBottom:24 }}>
        ...
        {step === 'done' && result && (
          <div style={{ background:'#ECFDF5', border:'1px solid #A7F3D0', borderRadius:12, padding:'20px 24px' }}>
            <div style={{ fontWeight:700, color:'#065F46', fontSize:15, marginBottom:12 }}>
              ✓ Successfully parsed and saved to Supabase
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12, marginBottom:16 }}>
              {[
                { label:'Total Rows',      value: result.row_count?.toLocaleString('en-IN') },
