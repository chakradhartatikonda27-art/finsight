      ...
      <div style={{ padding:'24px', fontFamily:'system-ui' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:16, marginBottom:24 }}>
          {[
            ...
            { label:'Balance Sheet Difference',  value:'₹0.00',          color:'#059669', sub:'BS001 PASS — perfectly balanced' },
          ].map(k => (
            <div key={k.label} style={{
              ...
              {k.sub && <div style={{ fontSize:11, color:'#059669', marginTop:4 }}>{k.sub}</div>}
            </div>
          ))}
        </div>
        ...
        <div style={{ background:'#fff', border:'1px solid #E5E7EB', borderRadius:12, overflow:'hidden', marginBottom:16 }}>
          ...
          {Object.entries(assetGroups).map(([category, items]: [string, any]) => (
            <div key={category}>
              <div style={{ background:'#F8FAFC', padding:'8px 20px', fontSize:11, fontWeight:700, color:'#6B7280', textTransform:'uppercase', letterSpacing:'.06em', borderBottom:'1px solid #F3F4F6', borderTop:'1px solid #F3F4F6' }}>
                {category}
              </div>
              {items.map((item: any) => (
                <div key={item.label} style={{ display:'flex', justifyContent:'space-between', padding:'11px 20px', borderBottom:'0.5px solid #F9FAFB', fontSize:13 }}>
                  <span style={{ color:'#374151' }}>{item.label}</span>
                  <span style={{ fontFamily:'monospace', fontWeight:500, color:'#111' }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', padding:'14px 20px', background:'#EFF6FF', fontWeight:700, fontSize:14 }}>
        ...
        <div style={{ background:'#fff', border:'1px solid #E5E7EB', borderRadius:12, overflow:'hidden', marginBottom:16 }}>
          ...
          {Object.entries(liabGroups).map(([category, items]: [string, any]) => (
            <div key={category}>
              ...
              {items.map((item: any) => (
                <div key={item.label} style={{ display:'flex', justifyContent:'space-between', padding:'11px 20px', borderBottom:'0.5px solid #F9FAFB', fontSize:13 }}>
                  <span style={{ color:'#374151' }}>{item.label}</span>
                  <span style={{ fontFamily:'monospace', fontWeight:500, color: item.amount.startsWith('-') ? '#DC2626' : '#111' }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', padding:'14px 20px', background:'#F5F3FF', fontWeight:700, fontSize:14 }}>
