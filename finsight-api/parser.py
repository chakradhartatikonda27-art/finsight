...
Fast version — batch size 1000, optimized for large files
"""
import openpyxl
import xlrd
import csv
...
def save_to_supabase(parsed: dict, org_id: str, upload_id: str, supabase_client) -> dict:
    """Fast batch insert — 1000 rows per batch."""
    sb = supabase_client
    vouchers = parsed['vouchers']
    inserted = 0
    BATCH = 1000  # increased from 200 to 1000

    for i in range(0, len(vouchers), BATCH):
        batch = [dict(v, org_id=org_id, upload_id=upload_id) for v in vouchers[i:i+BATCH]]
